...
#'   options for filling the data subdirectory. \cr \cr
...
#'   \code{metadata_control}, as well as items \code{cast_type}
#'   and \code{update} for simple use with all or ancillary data. \cr \cr
...
#' @export
#'
data_control <- function(cast_date = today(), 
                         ...
                         update = TRUE, override_ssl_cert_exp = TRUE,
  ...
  list(
    moons = 
      moons_control(n_future_moons = lead_time, cast_date = cast_date,
                    append_missing_to_raw = append_missing_to_raw,
                    save = m_save, filename = m_filename),
    rodents =
      rodents_control(cast_type = cast_type, min_traps = min_traps,
    ...
    covariates =
      covariates_control(cast_type = cast_type, cov_hist = cov_hist, 
...
moons_control <- function(n_future_moons = 12, cast_date = today(), 
                          ...
                          filename = "moons.csv", class = "moons"){
...
rodents_control <- function(cast_type = "forecasts",
                            ...
                            filename = "all.csv", class = "rodents"){
  list(cast_type = cast_type, start = start, end = end, min_traps = min_traps,
       ...
       level = level, plots = plots, output = output, save = save, 
       filename = filename, treatment = treatment, class = class)
}
