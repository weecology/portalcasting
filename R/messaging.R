...
#'
#' @description Messaging functions: \cr \cr 
#'              \code{version_message} creates a message for the local package  version. \cr \cr
#'              \code{portalcast_welcome} generates a welcome message for a portalcast run. \cr \cr
#'              \code{portalcast_goodbye} makes a goodbye message for when models are done. \cr \cr
#'              \code{sandbox_welcome} generates a special welcome message for sandboxes, displayed upon completion of \code{\link{setup_sandbox}}. \cr \cr
#'              \code{creation_message} makes a message about the creation of the directory. \cr \cr
#'              \code{setup_completion_message} creates a message for when the directory setting up has successfully finished. \cr \cr
#'              \code{fill_casts_message} creates the final message to be used in \code{\link{fill_casts}} that relays the number, and optionally the specific names, of the files moved. \cr \cr 
#'              \code{fill_fits_message} creates the final message to be used in \code{\link{fill_fits}} that relays the number, and optionally the specific names, of the files moved. \cr \cr
#'              
#' @param main \code{character} value of the name of the main component of the directory tree. 
#'
#' @param quiet \code{logical} indicator if message should be quieted.
#'
#' @param end_moon \code{integer} (or integer \code{numeric}) newmoon number of the last sample(s) to be included.
#'
#' @param files \code{character} vector of the base file names of the files that may or may not have been moved.
#'
#' @param movedTF \code{logical} vector indicating if each of the files in \code{files} has been moved or not.
#'
#' @param verbose \code{logical} indicator of whether or not to print out all of the files and whether they were moved or not.
#'
#' @param run_status Output from the \code{\link{source}}-ing of a model script; a \code{list} if successful or \code{NA} if not.
#'
#' @param model \code{character} value of the model name or model script path. Will be stripped of all excess path components and the extension to give just the name of the model itself. 
#' 
#' @return Message is put to the console; \code{NULL} is returned \code{\link[base]{invisible}}-ly.
#'
...
version_message <- function (quiet = FALSE) {
  ...
  messageq("This is portalcasting v", packageDescription("portalcasting", fields = "Version"), quiet = quiet)
  messageq(message_break(), quiet = quiet)
...
#' @export
#' 
creation_message <- function (main  = ".", 
                              quiet = FALSE) {

  messageq(message_break(), quiet = quiet)
  messageq("Establishing portalcasting directory at\n ", normalizePath(file.path(main = main), mustWork = FALSE), quiet = quiet)
  messageq(message_break(), quiet = quiet)
...
#' @export
#' 
fill_casts_message <- function (files   = NULL, 
                                movedTF = NULL, 
                                quiet   = FALSE,
                                verbose = FALSE) {
...
#' @export
#' 
fill_fits_message <- function (files   = NULL, 
                               movedTF = NULL, 
                               quiet   = FALSE,
                               verbose = FALSE){
