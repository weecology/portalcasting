...
#'  subsets.
#'
#' @param controls Control \code{list} (from \code{\link{rodents_control}}) or
#'  \code{list} of control \code{list}s (from \code{\link{rodents_controls}})
#'  specifying the structuring of the rodents tables. See
#'  \code{\link{rodents_control}} for details. 
#'
...
#' @export
#'
prep_rodents <- function(controls = rodents_controls(),
                         moons = prep_moons(main = main), main = ".", 
                         quiet = FALSE, overwrite = TRUE,
                         ref_species = all_species(), save = TRUE){
  ...
  ntmnt <- length(controls)
  table_args <- vector("list", length = ntmnt)
  table_args <- vector("list", length = ntmnt)
  for(i in 1:ntmnt){
    control_args <- controls[[i]]
    ...
    names(table_args)[i] <- controls[[i]]$name
  }
...
rodents_control <- function(tmnt_type = NULL, name = tmnt_type,
                            ...
                            filename = "rodents_all.csv", start_moon = 217,
                            end_moon = NULL, level = "Site", treatment = NULL,
                            plots = "all", min_plots = 24, 
  ...
  list(tmnt_type = tmnt_type, name = name, species = species, total = total,  
       filename = filename, start_moon = start_moon, end_moon = end_moon, 
       level = level, treatment = treatment, plots = plots, 
       min_plots = min_plots, min_traps = min_traps, output = output)
}
...
rodents_controls <- function(tmnt_type = c("all", "controls"),
                             name = tmnt_type,
                             species = base_species(), total = TRUE,
                             filename = NULL, start_moon = 217,
                             end_moon = NULL, level = NULL, treatment = NULL,
