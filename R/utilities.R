...
#'
#' @return \code{NULL} \code{\link[base]{invisible}}-ly, with the tmp subdirectory's files removed.
#'
#' @examples
#'  \donttest{
#'   create_dir()
...
#' @export
#'
clear_tmp <- function(main     = ".", 
                      settings = directory_settings(), 
                      quiet    = FALSE, 
                      verbose  = FALSE) {
  
  tmp_path   <- file.path(main, settings$sub$tmp)
  ...
  messageq(message_break(), "\nClearing tmp subdirectory", quiet = quiet)
  ...
  if (tmp_exist) {
    ...
    if(ntmp_files > 0){
      ...
      tmp_files_full_paths <- file.path(main, settings$sub$tmp, tmp_files)
      unlink(tmp_files_full_paths, force = TRUE, recursive = TRUE)

      messageq("    *temporary files cleared from tmp subdirectory*", quiet = !verbose)

    } else {

      messageq("    *tmp subdirectory already clear*", quiet = !verbose)

    }

  } else{

    messageq("    *tmp subdirectory not present for clearing*", quiet = !verbose)
