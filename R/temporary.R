...
update_forecasts_folder <- function (main = ".") {
  ...
  if (file.exists(file.path(main, "forecasts", "forecasts_metadata.csv"))) {
    forecasts_metadata <- read.csv(file.path(main, "forecasts", "forecasts_metadata.csv"))
  }

  old_colnames <- c("forecast_id", "forecast_group", "cast_date", "start_moon", "end_moon", "lead_time", "model", "dataset", "portalcasting_version", "QAQC", "notes")
  ...
  if (all(colnames(forecasts_metadata) %in% old_colnames)) {
    ...
    for (i in 1:ncasts) {
      ...
      lpath <- paste0("forecast_id_", forecasts_metadata$forecast_id[i], "_cast_tab.csv")
      ...
      lpath <- paste0("forecast_id_", forecasts_metadata$forecast_id[i], "_metadata.yaml")
      cpath <- file.path(main, settings$subdirectories$forecasts, lpath)
      ...
      cpath_json  <- file.path(main, settings$subdirectories$forecasts, paste0("forecast_id_", forecasts_metadata$forecast_id[i], "_model_casts.json"))
      cpath_RData <- file.path(main, settings$subdirectories$forecasts, paste0("forecast_id_", forecasts_metadata$forecast_id[i], "_model_casts.RData"))
      ...
      for (j in 1:nspecies) {
        ...
        forecast_id_j                   <- paste0(cast_tab_j$forecast_id[1], ".", ifelse(nchar(j) == 1, paste0(0, j), js[j]))
        cast_tab_j$old_forecast_id      <- cast_tab_j$forecast_id
        ...
        out_row <- data.frame(forecast_id                      = forecast_id_j,
                              old_forecast_id                  = forecasts_metadata$forecast_id[i],
                              forecast_group                   = forecasts_metadata$forecast_group[i],
                              cast_date                    = forecasts_metadata$cast_date[i],
                              origin                       = NA,
      ...
      lpath_json  <- paste0("forecast_id_", forecasts_metadata$forecast_id[i], "_model_casts.json")
      cpath_json  <- file.path(main, settings$subdirectories$forecasts, lpath_json)

      lpath_RData <- paste0("forecast_id_", forecasts_metadata$forecast_id[i], "_model_casts.RData")
    ...
    meta_path <- file.path(main, settings$subdirectories$forecasts, settings$files$foreforecasts_metadata)
    row.names(out) <- NULL    
