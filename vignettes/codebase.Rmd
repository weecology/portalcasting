...
└──casts
│   <previous and current model output>

```

(**Note**: Previous versions of **portalcasting** had an explicitly named `base` level more basal than `main`, but that has been removed as of v0.9.0 due to its redundancy. To group the project subfolders into a multi-leveled folder, simply add structure to the `main` input, such as `main = "~/project_folder"`.)
...
`create_dir` also initializes the `dir_config.yaml` file, which is held within `main` and contains directory setting up metadata.
...
* `fill_raw` downloads each of the raw components for the directory, such as the source data and previous forecasts. Upon completion of the downloads, `fill_raw` updates `dir_config.yaml` with download versions.
