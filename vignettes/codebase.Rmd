...
are flexible, as detailed in Options List.
...
list using `main_path()` and `sub_paths()`. In addition, `model_paths()` 
can be used to find a specific model's (or models') full path(s)
and `file_paths()` can be used to find the full path of any file(s) within
the directory.
...
some level of data and forcast processing. The default settings for each
