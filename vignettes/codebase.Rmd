...
<function pipeline> with explanations like above, not like before
...
`check_args` provides important backstopping and error checking of a standard *portalcasting* production pipepline. However, it is likley that a user in development spaces (a.k.a. sandboxes) would like to deviate from the standard behavior to leverage, for example, the ability to construct novel rodent `tmnt_types` (groups of observations over time). To that end, we incldue an argument `arg_checks` that is in nearly every function throughtout the codebase, which flips a switch and turns off the vast majority of the *portalcasting*-generated errors. In this space, the user will be able to create a much broader array, but will likely be first informed of errors through more base functions. To that end, in default sandboxing mode, we set `arg_checks = FALSE` (minimal checking from *portalcasting*) and `verbose = TRUE` (maximum feedback from underlying functions).
