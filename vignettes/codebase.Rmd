...
**portalcasting** has a generalized `read_data()` function that allows for toggling among `read_rodents()`, `read_rodents_dataset()`, `read_covariates()`, `read_newmoons()`, and `read_metadata()`, which each have specific loading procedures in place. Similar to the `read_data()` functions, `read_casts()` provides a simple user interface for reading the cast files into the R session.
