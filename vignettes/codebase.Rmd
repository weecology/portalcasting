...
A typical user is likely to want to change the `main` input (to locate the forecasting directory where they would like it), but general users should not alter the `subs` structure, and so that option is not easily available. 
If needed, the `subs` can be altered via the `directory_settings` controls.
