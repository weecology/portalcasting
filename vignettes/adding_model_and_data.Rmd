...
```{r, eval=FALSE}
newmod_controls <-  model_script_control(name = "newmod", data_sets = "newdata", covariatesTF = TRUE, lag = 0, arg_checks = TRUE)
```

And then you can run the `portalcast` function with the arguments specified as needed:

```{r, eval=FALSE}
portalcast(main = main, models = "newmod", controls_models = newmod_controls, controls_rodents = newdata_controls, arg_checks = FALSE)
```
...
2. Fill script with model code.
    + Create a model control list called `controls_newmod` using `model_script_control`
3. Add "newdata"
