...
Presently, the preloaded model set includes [seven models](https://weecology.github.io/portalcasting/articles/models.html): ESSS, AutoArima, NaiveArima, nbGARCH, nbsGARCH, pevGARCH, and jagsRW. The jags and GARCH models all take a bit longer to run (nbGARCH less time than nbsGARCH less time than pevGARCH, all less than jagsRW), so for the purposes of illustration, we will only run ESS, AutoArima, and NaiveArima models, which we indicate through input to the `models` argument: 
