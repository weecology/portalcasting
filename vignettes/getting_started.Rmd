...
Presently, the preloaded model set includes `r nmodels_text` models: `r models_names`. The jags and GARCH models all take a bit longer to run, so for the purposes of illustration, we will only run ESS, AutoArima, and NaiveArima models, which we indicate through input to the `models` argument: 
