...
By default, `setup_dir()` downloads the most up-to-date version of both the [Portal Data](https://doi.org/10.5281/zenodo.1215988) and [Portal Predictions](https://doi.org/10.5281/zenodo.833438) archives from Zenodo and the NMME downscaled weather forecasts into the `resources` subdirectory; unpacks relevant components; prepares the newmoons, rodents, covariates, covariate forecast, and metadata data files (and places them in the `data` subdirectory); and populates the `models` subdirectory with scripts for the `r nmodels_text` existing models (`r model_names`).
