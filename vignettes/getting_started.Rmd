...
read_foreforecasts_metadata(main = main)
```
...
`select_forecasts` provides a simple interface to the cast metadata file with quick filtering:

```{r, eval=FALSE}
select_forecasts(main = main, models = "AutoArima")
```

And `read_cast_tab`, `read_model_cast`, and `read_foreforecasts_metadata` each read in the associated output from a given cast, as indicated by its forecast_id, which is displayed in the output from `select_forecasts`:

```{r, eval=FALSE}
read_cast_tab(main = main, forecast_id = "1.1")
read_model_cast(main = main, forecast_id = "1.1")
read_cast_metadata(main = main, forecast_id = "1.1")
```
