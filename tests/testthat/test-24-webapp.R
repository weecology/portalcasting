...
test_that(desc = "portal_forecast_ui builds a ui", {
  ...
  expect_message(ui <- portal_forecast_ui(main = main))
  expect_is(ui, "list")
...
test_that(desc = "panels build tags", {
  ...
  expect_silent(mp <- page_main_panel(main = main))
...
test_that(desc = "forecast tabs build tags", {
  ...
  expect_silent(ftrh <- forecast_tab_input_selection_row_historic_end_newmoonnumber(main = main))
...
test_that(desc = "evaluation tabs build tags", {
  ...
  expect_silent(ft <- evaluation_tab(main = main))
  expect_is(ft, "shiny.tag")

  expect_silent(ftcr <- evaluation_tab_input_selection_checks_row())
  expect_is(ftcr, "shiny.tag")

  expect_silent(ftr <- evaluation_tab_input_selection_row(main = main))
  expect_is(ftr, "shiny.tag")
  ...
  expect_silent(ftrh <- evaluation_tab_input_selection_row_historic_end_newmoonnumber(main = main))
  expect_is(ftrh, "shiny.tag")
...
test_that(desc = "lists functions build lists", {
  ...
  expect_silent(ft <- historic_end_newmoonnumber_list(main = main))
