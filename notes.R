...
### Major API updates
* The API has undergone substantial changes to functions and arguments.
* Default top-level behavior has been retained, but the API now provides 
improved interactivity.

#### `setup_dir`
* `models`, `tree`, and `quiet` are the top-level arguments not within control
lists. 
  * `models` takes a character vector of model names to use to create the models
subdirectory. Default value is the output from `model_names()`, which is the
prefab model set. The output from `model_names()` can be overwritten in the 
first argument, but `model_names()` is not needed for input to `models`, which
only requires its input to be a character vector.
  * `tree` takes a directory tree list as output from `dirtree()` with specific
inputs to `base`, `main`, and `subs` arguments if desired.
  * `quiet` is a `logical` argument that controls all message output in itself 
and all contained functions. 
* `PortalData_control`, `data_control`, `predictions_control` and `models_control`
are top-level control arguments that take lists of parameters. Each has a function
of the same name that defines the possible inputs and their default values 
(used if not explicitly overwritten). Most controls are as they were in v0.8.0,
with some additions, but orders have been re-arranged, so it is possible
users may need to edit argument lists.
  * `models_control`, however, is set up a bit differently and is 
a hybrid of how the individual `<modelname>_options` functions previously operated.
Now, however, there's a single `model_options` function that allows crafting of the 
list of controls for a single model from a simple interface. `models_control`
takes an argument for each of the prefab models and allows for expansion to other
models via elipses (`...`), which means that additional models should be input
as `list`s or as outputs from `model_control`.
  * There is no longer a wrapping of all of the control options into a single list
("`all_options`").

#### `setup_sandbox`
* A new function that wraps around `setup_dir` with defaults useful for
sandboxing, rather than in the production pipeline.
* Presently, the only difference is the download of the historic forecasts
(rodent predictions and covariate forecasts)... oh and a cute picture to
soften the ending of the messages if you don't quiet things!

#### `create_dir` suite of functions: `create_dir`, `create_base_dir`,
`create_main_dir`, `create_sub_dirs`
* Each function takes `tree` and `quiet` arguments as defined above.
* At each level in the hierarchy, if the more basal levels do not exist, an error is thrown

#### `fill_dir` suite of functions: `fill_dir`, `fill_PortalData`, `fill_data`,
`fill_predictions`, `fill_models`
* `fill_dir` has the same argument inputs as `create_dir`
* Subfunctions within `fill_dir`: `fill_PortalData`, `fill_data`, `fill_predictions`,
`fill_models` each call their respective control list just `control`.
* The `fill_` functions' controls now also allow for an `update` argument (defaults
to `TRUE`), which can be flipped to prevent updating of old files.

#### `prep_` data functions: `prep_moons`, `prep_rodents_list`, `prep_covariates`, 
`prep_metadata`
* All now take `tree` and `quiet` as explicit arguments and then take a control list
to the `control` argument in replacement for the old `options` list argument.
* The same goes for functions within the `prep_` data functions.

#### `portalcast`
* `models`, `tree`, and `quiet` are the top-level arguments not within control
lists. 

### Removal of `all_options` `dir_options` without `_control` replacements
* Due to the elevation of `tree` and `quiet`, the control lists were minimized to
remove redundancies.  
* All other `_options` functions have `_control` replacements

### New download capacity for existing predictions and covariates
* Both `download_hist_covariate_forecasts` and `download_predictions` allow for users to
collect the most recent, archived predictions and covariates forecasts for local use.
* `download_hist_covariate_forecasts` replaces `transfer_hist_covariate_forecasts`,
which only had the option of a local copy of historical covariates from within the 
package, with a function that can do that, but defaults to downloading from
zenodo the most recent archived data file, and it can also get it from github
(by turning `from_zenodo` to `FALSE` in the control list).
* `download_predictions` now has a smooth download from the zenodo archive as
its default, with the option for the github download available still 
via the control list.
* Associated utilities include `get_releases`, `get_latest_zenodo_release`, and 
`get_github_releases`

#### Control arguments lists saved as files
* The functions that fill folders with data sets now write out their controls
as `.yaml` files to minimize user inputs needs.
* As a result, the `portalcast` function has much simpler interface.

### Other new functionality or new functions
* `create_base_dir` 
* `create` is generalized
* `verify`
* `setup_sandbox`
* `check_url`
* `ifnull`
    ...
    local_path <- paste0("data/PortalData_control.yaml")
    con_path <- file_paths(tree, local_path)
    writeLines(as.yaml(control), con = con_path)
...
DO NOT LOSE THIS
oh it should be a top level argument right, and hindcasts and 490:403
  if (is.null(end) & cast_type == "hindcasts"){
    end <- 490:403
  }

reset_dir
hindcast as a wrapper function on portalcast
default set cast_type and end
...
done with the creation and the filling!
setup is good!
now on to portalcast!
...
models = model_names(), covariates = NULL,
                        lag = NULL, trim = FALSE, quiet = FALSE){
  mm <- models_meta_prefab
  pmm <- names(models_meta_prefab)
  npmm <- length(pmm)
  unames <- unique(c(models, pmm))
  nupmm <- length(unames)
  newmods <- models[!(models %in% pmm)]
  nnewmods <- nupmm - npmm 
  existmods <- models[(unames %in% pmm) & (unames %in% models)]
  nexistmods <- length(existmods)   

  if(is.null(covariates) & !is.null(models) & nnewmods > 0){
    modnames <- paste(newmods, collapse = ", ")
    msg <- paste0(" *assuming no covariates (and no lag) for ", modnames, "*")
    messageq(msg, quiet)
    lag <- NULL
  } else if (is.null(lag) & !is.null(models) & nnewmods > 0){
    modnames <- paste(newmods, collapse = ", ")
    msg <- paste0(" *assuming no lag = 0 for ", modnames, "*")
    messageq(msg, quiet)
    lag <- rep(0, length(models))
  }
  covariates <- ifnull(covariates, rep(FALSE, length(models)))
  lag <- ifnull(lag, rep(FALSE, length(models)))

  if(nnewmods > 0){
    for(i in 1:nnewmods){
      spec <- which(models == newmods[i])
      entry <- list(list(models = newmods[i], covariates = covariates[spec],
                         lag = lag[spec]))
      mm <- append(mm, entry)
      names(mm)[length(mm)] <- newmods[i] 
    }
  }
  if(nexistmods > 0){
    for(i in 1:nexistmods){
      spec <- which(pmm == existmods[i])
      entry <- list(models = models_meta_prefab[[spec]]$name, 
                    covariates = models_meta_prefab[[spec]]$covariates,
                    lag = models_meta_prefab[[spec]]$lag)
      spec2 <- which(names(mm) == existmods[i])
      mm[[spec2]] <- entry
    }
  }
  if(trim){
    mm2 <- mm
    keeps <- which(names(mm2) %in% models)
    nkeeps <- length(keeps)
    mm <- vector("list", length = nkeeps)
    if(nkeeps > 0){
      for(i in 1:nkeeps){
        mm[[i]] <- mm2[[keeps[i]]]        
      }
      names(mm) <- names(mm2)[keeps]
    } else{
      mm <- NULL
    }
  }
...
thought: i'd love to have fill_PortalData be able to determine if it 
actually needs to download or not
